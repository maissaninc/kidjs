(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.KID = {}));
  ...
  Animation.prototype = {
    ...
    onRequestAnimationFrame: function onRequestAnimationFrame() {
  ...
  function Canvas(id) {
    this.id = id;
    this.imageCache = {}; // Insert canvas element into DOM
    ...
    this.context = this.canvas.getContext('2d'); // Canvas methods

    this.methods = ['drawImage', 'moveTo', 'lineTo', 'line', 'bezierCurveTo', 'rect', 'fillRect', 'clearRect', 'clear', 'arc', 'circle', 'fillCircle', 'write', 'writeln', 'putpixel']; // Set screen width and height values
  ...
  Canvas.prototype = {
    constructor: Canvas,
    addMethodsToScope: function addMethodsToScope(scope) {
    ...
    loadImage: function () {
      var _loadImage = _asyncToGenerator(function* (image) {
        // Check for image in cache
        if (this.imageCache[image]) return; // Apply filter to image URL
        ...
        return new Promise(function (resolve, reject) {
          ...
          imageToLoad.onload = /*#__PURE__*/_asyncToGenerator(function* () {
    ...
    drawImage: function drawImage(image, x, y, w, h) {
      // Image object passed
      if (image instanceof Image) {
        if (typeof w !== 'undefined' && typeof h !== 'undefined') {
          this.context.drawImage(image, this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('x', w), this.scalarUnit('y', h));
        } else {
          this.context.drawImage(image, this.scalarUnit('x', x), this.scalarUnit('y', y));
        }
      } // Image URL passed
    ...
    bezierCurveTo: function bezierCurveTo(x1, y1, x2, y2, x3, y3) {
      ...
      this.context.beginPath();
      this.context.moveTo(this.pathX, this.pathY);
      this.context.bezierCurveTo(this.scalarUnit('x', x1), this.scalarUnit('y', y1), this.scalarUnit('x', x2), this.scalarUnit('y', y2), this.scalarUnit('x', x3), this.scalarUnit('y', y3));
      this.pathX = this.scalarUnit('x', x);
      this.pathY = this.scalarUnit('y', y);
      this.context.stroke();
    },
    rect: function rect(x, y, w, h) {
      this.setProperties();
      this.context.beginPath();
      this.context.rect(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('w', w), this.scalarUnit('h', h));
      this.context.stroke();
    },
    fillRect: function fillRect(x, y, w, h) {
      this.setProperties();
      this.context.beginPath();
      this.context.fillRect(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('w', w), this.scalarUnit('h', h));
    },
    clearRect: function clearRect(x, y, w, h) {
      this.setProperties();
      this.context.clearRect(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('x', w), this.scalarUnit('y', h));
    },
    ...
    arc: function arc(x, y, r, start, stop) {
      this.setProperties();
      this.context.beginPath();
      this.context.arc(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('r', r), this.degreesToRadians(start), this.degreesToRadians(stop));
      this.context.stroke();
    },
    ...
    fillCircle: function fillCircle(x, y, r) {
      this.setProperties();
      this.context.beginPath();
      this.context.arc(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('r', r), 0, Math.PI * 2);
      this.context.fill();
    },
    ...
    putpixel: function putpixel(x, y, color) {
      var temp = this.context.fillStyle;
      this.context.fillStyle = color;
      this.context.fillRect(this.scalarUnit('x', x), this.scalarUnit('y', y), this.scalarUnit('x', 1), this.scalarUnit('y', 1));
      this.context.fillStyle = temp;
    },
    scalarUnit: function scalarUnit(key, value) {
      ...
      if (match[2] == '%' && key == 'r') {
        return parseFloat(match[1]) / 100 * Math.min(window.innerWidth, window.innerHeight);
      }
    },
    setProperties: function setProperties() {
      var properties = ['fillStyle', 'strokeStyle', 'shadowColor', 'shadowBlur', 'shadowOffsetX', 'shadowOffsetY', 'lineCap', 'lineJoin', 'lineWidth', 'miterLimit', 'font', 'textAlign', 'textBaseline'];
  ...
  Debug.prototype = {
    constructor: Debug,
    step: function () {
      var _step = _asyncToGenerator(function* (line, column) {
    ...
    _createStepStatement: function _createStepStatement(location) {
      return {
    ...
    _createEndStatement: function _createEndStatement() {
      return {
        type: 'ExpressionStatement',
        expression: {
          type: 'CallExpression',
  ...
  DeviceOrientation.prototype = {
    ...
    getHorizontalTilt: function getHorizontalTilt(e) {
  ...
  function Pointer() {
    this.x = false;
    this.y = false;
    this.t = false; // Track mouse poistion in global variable
  ...
  Pointer.prototype = {
    ...
    onPointerUp: function onPointerUp(e) {
      if (this.x !== false && this.y !== false && this.t !== false) {
        ...
        var deltaT = Date.now() - this.t; // Determine swipe
  ...
  Permissions.prototype = {
    ...
    requestPermissions: function () {
      var _requestPermissions = _asyncToGenerator(function* () {
        ...
        while (permission = this.requested.pop()) {
          switch (permission) {
            case 'deviceorientation':
              var state = yield DeviceOrientationEvent.requestPermission();
            ...
            case 'devicemotion':
              var state = yield DeviceMotionEvent.requestPermission();
    ...
    clickToStart: function clickToStart(callback) {
    ...
    var _ref = _asyncToGenerator(function* (code) {
      ...
      simple(ast, {
        CallExpression: function () {
          var _CallExpression = _asyncToGenerator(function* (node) {
      ...
      ast.body[0].expression.callee.body.body.push(KID.debug._createEndStatement()); // Generate code

      code = generate(ast); // Permissions required
